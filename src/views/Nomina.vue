...
const { capitalize } = useMiscellaneous();
      ...
      <v-table>
        ...
        <tbody>
          <tr v-for="employee in employees" :key="employee.name">
            <td>
              <v-avatar v-if="employee.image">
                <img alt="Avatar" :width="50" :src="employee.image" />
              </v-avatar>
              <v-avatar v-else class="text-white" color="#1B76BC">{{
    ...
    <v-dialog v-model="dialog" width="auto">
      <base-card>
        ...
        <template v-slot:actions>
          ...
          <v-btn color="#016ff6" text @click.stop="dialog = false">
            Salir
          </v-btn>
        </template>
      </base-card>
